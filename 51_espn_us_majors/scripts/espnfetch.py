...
Shared in substance with `65_espn_college/scripts/espnfetch.py` — same endpoints, same
rightsholder, same throttling behaviour — and kept as a per-package copy rather than a shared
module because the two packages disagree about the one thing that matters here: the scoreboard
params, and therefore the cache key. See `scoreboard_rel`.
...
def scoreboard_rel(lg: dict, window: str) -> str:
    """Cache path for one scoreboard window, keyed on league AND params.

    `window` is whatever went into `dates=`: a single day (`20240110`) or a range
    (`20240101-20240131`). This package discovers by monthly RANGE, where `&limit=1000` is
    REQUIRED — without it the range endpoint truncates at 100 events and still returns HTTP 200.
    That is the exact opposite of `65_espn_college`, where the same param truncates a single-date
    query. Two packages, one endpoint, opposite correct answers: hence the params in the key.
    """
    return f"census/scoreboard/{lg['league']}/{params_tag(lg.get('params', ''))}/{window}.json"


def census_cell_rel(lg: dict, season: int) -> str:
    """Cache path for one (league, season) census cell, keyed on the league LABEL.

    The label rather than the ESPN slug because a label can legitimately be a tier of a shared
    slug (in `65` FBS and FCS are both `college-football`). NBA/NFL/NHL are one label per slug, so
    the two agree here — the label is used anyway so the two packages' cells are named alike.
    """
    return f"census/seasons/{lg['label']}_{season}.json"


def find_census_cell(cache: Path, lg: dict, season: int) -> Optional[Path]:
    """The cell for this (league, season), or None. Read-side counterpart of `census_cell_rel`."""
    fp = Path(cache) / census_cell_rel(lg, season)
